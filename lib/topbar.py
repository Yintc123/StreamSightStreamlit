...
ThemeToggle SVG 對齊 ThemeToggle.tsx light mode（circle r=5 + 8 條 line）。
"""
...
def _build_topbar_html(actor: Actor, cms_base_url: str = "") -> str:
    ...
    return (
        ...
        f'<button class="ss-topbar__theme-btn" type="button" aria-label="切換為深色">'
        f"{_SUN_SVG}</button>"
...
def render_topbar(actor: Actor, cms_base_url: str = "") -> None:
    ...
    """
    st.markdown(_build_topbar_html(actor, cms_base_url), unsafe_allow_html=True)
