...
- **Streamlit → BFF `POST /api/auth/logout`**:unsafe method,需帶 `X-CSRF-Token` 與 `Origin` header。csrfToken **由 introspection 一併回傳**,存 `session_state["csrf_token"]`(`resolve_actor` 落地時寫入);`_do_logout_bff()` 從 `state.get_csrf()` 取用。Origin 必須在 BFF `ALLOWED_ORIGINS` 白名單中，由 `config.streamlit_origin` 設定(預設 `http://localhost:8501`)。**已定案(2026-07-18)**;見 015 §2.3、§7.1A/B。
