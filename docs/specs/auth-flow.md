...
- 狀態:規劃中(v2 — 依實際 BFF 架構重寫);**資料存取模式已定案採 Design B**,見 [ADR 0003](../decisions/0003-auth-via-bff-token-exchange.md)
...
> **信任邊界說明**:此端點會把 access token 交給 Streamlit 伺服器。這是**刻意的取捨**——[ADR 0002] 本就設計 Streamlit 直連 FastAPI 並保管 JWT,故 token 進入 Streamlit server 記憶體的風險與現況一致,且**仍未進入瀏覽器**(比舊 JWT-in-session_state 方案更佳)。若要求 token **絕不離開 BFF**,改採 §5「Design A」(代價:前端要 proxy 全部資料端點)。
...
- [ ] introspection 回應是否**含 `accessToken`**(Design B 需要;Design A 不需要)。
