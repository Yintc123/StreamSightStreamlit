...
- `grade`:後端 JWT grade claim(admin → `super_admin` / `editor` / `viewer`);為**存取軸**,Streamlit 端原樣帶入 `Actor.grade`,供 `can_write`(`grade != "viewer"`)判斷。見[前端頁面結構 §存取控制](frontend-pages.md#存取控制本節為存取軸的單一真相)。
