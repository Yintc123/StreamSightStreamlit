...
狀態：**已實作（2026-07-19）**（v0.3；Frontend 019 已實作、根 `.env` 開關已啟用，僅餘 §4 跨 app 手動驗收）
...
| `parse_sidebar_width(raw) -> int \| None` | 純函式：**ASCII** 十進位整數字串且在 `[200, 600]` → int；其餘（`None` / 空 / 非數字 / 越界 / 浮點 / 非 ASCII 數字）→ `None`。與 JS `valid()`（regex `\d` 僅 ASCII）同語義，供測試錨定契約 |
...
| 0.3 | 2026-07-19 | **已實作**：`lib/sidebar.py`（常數 / `parse_sidebar_width` / `build_sidebar_sync_js` / `inject_sidebar_sync_js`）＋ `test_sidebar.py` 16 案、`config` 旗標 +2 案、`app.py` ⑦″ 接線；JS 與 §3.2 逐字一致。規格核對修正：`parse_sidebar_width` 加 `isascii` 檢查（上標數字 `isdigit` 為 True 但 `int()` 丟 ValueError、全形數字 JS `\d` 不收——「其餘 → None」語義補密合，§3.3 改述「ASCII 十進位整數字串」）。根 `.env` / `.env.example` 已加 `ENABLE_SIDEBAR_WIDTH_SYNC=1`（§3.4）。全套件 pytest 500 綠。僅餘 §4 跨 app 手動驗收（需雙端同開實機操作）。 |
