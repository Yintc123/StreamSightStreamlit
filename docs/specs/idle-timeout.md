...
- 狀態：**已實作**（2026-07-19，嚴格 TDD）。純函式 `lib/idle.py` + `app.py` 接線；伺服器端兜底依 D2 不做。
...
| `app.py` | ① `inject_idle_js()`（於 `inject_theme_js()` 之後，冪等）；② 既有 `?logout=1` 分支解析 `reason` 並**就地分岔**（見 §7.2；未抽 helper，維持 app.py 薄線性腳本、與原內聯 logout 區塊一致）；③ 於 `resolve_actor()` 之後、登出偵測**之前**加入 `_logout_reason` 顯示區塊（見 §7.1） | AppTest |
...
if pop_logout_reason() == "idle":            # 讀後即清
    st.toast("因閒置逾 15 分鐘，已登出", icon="⏱️")

# ④′ 登出偵測（既有 ?logout=1 分支，就地擴充解析 reason；依 §7.2 分岔）
if st.query_params.get("logout") == "1":
    ...
    if _s.use_mock:
        if reason == "idle":
            set_logout_reason("idle")        # 跨 rerun 存活，下一輪顯示 toast
        st.query_params.clear()
        st.rerun()
    else:
        _login_url = _s.bff_login_url
        if reason == "idle":                 # 閒置：過場頁 + 延時 3s 跳轉
            st.info("因閒置逾 15 分鐘，已將您登出，正在導向登入頁…")
            st.markdown(f'<meta http-equiv="refresh" content="3; url={_login_url}">', unsafe_allow_html=True)
        else:                                # 手動登出：即時跳轉（content=0）
            st.markdown(f'<meta http-equiv="refresh" content="0; url={_login_url}">', unsafe_allow_html=True)
        st.stop()

# ⑤ 開發切換器（不變）...
inject_theme_js()
inject_idle_js()                    # 於 inject_theme_js 之後，冪等注入
```
...
### 7.2 登出分支的 reason 分岔（就地擴充，未抽 helper）

現行 `app.py`（`logout.md §4.2`）登出後：mock 清 param＋rerun；bff 即時 `meta refresh content="0"`。本規格在**既有登出分支內就地擴充**（不抽獨立函式，維持 app.py 薄線性腳本、與原內聯 logout 區塊風格一致），**依 reason 分岔**，並確保**手動登出行為不變**：
