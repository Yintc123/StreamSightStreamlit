...
- **存取控制**(權威定義見[前端頁面結構 §存取控制](frontend-pages.md#存取控制本節為存取軸的單一真相)):本系統為 **admin-only**,`role` 恆為 `"admin"`,故**四頁皆註冊給所有登入者**。`if role == "admin"` 這道 role gate 在本部署恆為真,保留作為 `role == "user"` 若出現時的 **latent 深度防禦**(不是隱藏連結)。
