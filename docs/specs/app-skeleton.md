...
try:
    ...
    nav = st.navigation(pages)                          # ⑧ 交給 Streamlit 路由
    with page_shell(nav.title):                         #    以當前頁 title 為 key 包穩定容器(防殘影)
        nav.run()
...
- ⑧ 路由頁以 `page_shell(nav.title)` 包一層**帶頁面專屬 key 的容器**再 `nav.run()`。目的：消除**跨頁殘影（ghosting）**——Streamlit 前端對 element tree 做「位置＋型別」的增量 diff，切頁時同位置、同型別的舊 element 會被就地重用/暫留，直到整輪跑完才修剪，使前一頁元件短暫（或因新頁提早 `st.stop()`/例外而持續）殘留在新頁；容器帶頁面專屬 key 後，該區塊 `proto.id` 編入 `page-<title>`，切頁時 key 不同 → 整塊 remount 取代而非同位置重用（見 [ui.md `page_shell`](ui.md)）。`nav.title` 每頁唯一且非空（預設頁 `url_path` 為 `''`，故不以 url_path 當 key）。
