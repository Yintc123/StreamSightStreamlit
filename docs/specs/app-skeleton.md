...
from lib.request_id import init_logging       # 掛 streamsight.api filter(request-id §4.2)
...
- **存取控制**(權威定義見[前端頁面結構 §存取控制](frontend-pages.md#存取控制本節為存取軸的單一真相)):本系統為 **admin-only**,`role` 恆為 `"admin"`,故**五頁皆註冊給所有登入者**。`if role == "admin"` 這道 role gate 在本部署恆為真,保留作為 `role == "user"` 若出現時的 **latent 深度防禦**(不是隱藏連結)。
- **系統管理頁的存取差異在 grade,不在 role**:三種 grade 皆可讀,寫入限 `grade != "viewer"`(`can_write(actor)`);此行為需 AppTest 覆蓋(viewer 進得來但寫入按鈕停用)。
...
- **app**(`tests/app/`):進站顯示導覽、切換 grade 後寫入按鈕停用(viewer)、系統管理頁三 grade 皆可讀且 viewer 寫入停用、bff 未登入停在導向頁。
...
4. `lib/nav.py`:`build_pages(role)`——admin 註冊系統管理頁;`user` 分支為 latent 防線(本部署恆 admin)。(unit)
...
- 系統管理頁三 grade 皆可讀;viewer 寫入按鈕停用。
- `pytest` 全綠。
...
- ~~頁面檔名前綴是否統一~~ **已定**:採 CLAUDE.md 無編號慣例(`dashboard.py`/`data_management.py`/`realtime_monitor.py`/`analytics.py`/`admin.py`),順序由 `build_pages` 顯式決定,不靠檔名。`gate.py` 已刪除,未登入邏輯收進 `app.py`。
