...
3. **模式旗標正交**:資料來源(`DATA_SOURCE`)與認證(`AUTH_MODE`)各自可切 mock/真實;dev 預設全 mock,**不依賴 BFF 與後端即可跑**。
...
flowchart TB
    ...
    subgraph Identity["身分出口 lib/auth.py"]
...
- **Dev 設定檔(全 mock)**:兩者皆 `mock` → 無需登入、無需後端,進站即有假身分與假資料。
- **正式(全真實)**:`DATA_SOURCE=api` + `AUTH_MODE=bff`。
- 兩旗標可獨立組合(如 `mock` 資料 + `bff` 認證)以利分階段整合。
...
if actor is None:                                       # ④ 未登入(僅 AUTH_MODE=bff 會發生)
    st.navigation([st.Page("pages/01_gate.py")]).run()  #    只註冊導向頁 → 結束
    st.stop()
...
def build_pages(role: str) -> list:
    pages = [
        st.Page("pages/02_dashboard.py",       title="儀表板",   default=True),
        st.Page("pages/03_data_management.py", title="資料管理"),
        st.Page("pages/04_realtime.py",        title="即時監控"),
        st.Page("pages/05_analytics.py",       title="資料分析"),
    ]
    if role == "admin":                        # 非 Admin 動態不註冊(不是隱藏)
        pages.append(st.Page("pages/06_admin.py", title="系統管理"))
    return pages
```
...
app.py                      # 進入點(§3)
pages/
├── 01_gate.py              # 未登入導向頁(僅 bff 未登入時註冊)
├── 02_dashboard.py         # 儀表板(骨架可為 placeholder)
├── 03_data_management.py   # 資料管理(第一個實作的業務頁)
├── 04_realtime.py          # 即時監控(placeholder)
├── 05_analytics.py         # 資料分析(placeholder)
└── 06_admin.py             # 系統管理(僅 Admin)
lib/                        # 見 §6
...
6. `MockDataSource.list_records` + `pages/03`:資料管理頁能列出種子資料。(unit + app)
...
- `st.navigation` 的頁面檔名前綴(`01_`…)與 `pages/` 既有命名(見 frontend-pages)是否統一——實作時對齊一種即可。
